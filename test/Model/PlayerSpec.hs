...
spec = do
  ...
  describe "addPlayerToTeam" $ do
    it "is should add player to the beginning of the player's list" $ property $
      \p t -> head (teamPlayers $ addPlayerToTeam p t) == p

    it "is should increment team's goals count by the player's goals count" $ property $
      \p t -> teamGoalsCount (addPlayerToTeam p t) - playerGoalsCount p == teamGoalsCount t

    it "is should increment team's goals quota by the player's goals quota" $ property $
